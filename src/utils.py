...
def load_test_data_exr(image_path, h=None, w=None):
    img = read_exr(image_path)
    if h != None and w != None:
        img = cv2.resize(img, (w, h))
    img = hdr_to_ldr(img, gamma=1, dtype='uint8', clamp=True)
    img = img * 2 - 1
...
def save_images_exr(images, image_path):
    write_exr(image_path, images)
